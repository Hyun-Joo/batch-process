...
public class ItemWriterConfiguration {
    ...
    private ItemWriter<Person> jpaItemWriter() throws Exception {
        JpaItemWriter<Person> itemWriter = new JpaItemWriterBuilder<Person>()
                ...
                .usePersist(true)   // 해당 설정이 없으면 기본 merge 메소드 실행(dml과 select를 한꺼번에 실행)
                .build();
    ...
    private ItemWriter<Person> csvFileItemWriter() throws Exception {
        ...
        FlatFileItemWriter<Person> itemWriter = new FlatFileItemWriterBuilder<Person>()
                ...
                .append(true)   // 기존 파일에 덮어쓰지 않고 새로운 데이터 append하기
                .build();
        itemWriter.afterPropertiesSet();
    ...
    private List<Person> getItems() {
        ...
        for(int i = 0; i < 100; i++){
            //items.add(new Person(i + 1, "test name" + i, "test age", "test addr"));
            // id를 직접 할당하지 않아 merge 대상으로 인식하지 않게 함
            items.add(new Person("test name" + i, "test age", "test addr"));
        }
        return items;
    }
}
